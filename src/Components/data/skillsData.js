import React from 'react';
import Tooltip from '@material-ui/core/Tooltip';
import IconButton from '@material-ui/core/IconButton';
import Avatar from '@material-ui/core/Avatar';
import ListItem from '@material-ui/core/ListItem';
import ListItemIcon from '@material-ui/core/ListItemIcon';
import ListItemText from '@material-ui/core/ListItemText';
import LanguageIcon from '@material-ui/icons/Language';
import EditIcon from '@material-ui/icons/Edit';
import PhotoSizeIcon from '@material-ui/icons/PhotoSizeSelectActual';

const iconMargin = {
    margin: '4px'
}


const devToolsData = [
    {name: "Github", link:"https://image.ibb.co/kxxjck/github.png"},
    {name:"NPM", link:"https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTF6m4oGLOX6fwd26f018WRjUm4HXYGJEcGe20xkPKDDWPTdlOx"},
    {name:"Sass", link:"https://image.ibb.co/k5Wcxk/sass.png"},
    {name:"Babel", link:"https://cdn-images-1.medium.com/max/2000/1*XmHUL5DeySv_dGmvbPqdDQ.png"},
    {name:"Webpack", link:"https://raw.githubusercontent.com/webpack/media/master/logo/icon-square-big.png"}
]

const otherData = [
    {name: "MySQL", link:"https://i.pinimg.com/originals/96/b6/6f/96b66fc862b74edd5ad9e89cc7ccdbb0.png"},
    {name:"MongoDB", link:"https://image.ibb.co/eioeA5/mongodb.png"},
    {name:"Photoshop", link:"https://image.ibb.co/b04mq5/photoshop.png"}
]

const frameworkLibraryData = [
    {name: "ExpressJS", link:"https://d2eip9sf3oo6c2.cloudfront.net/tags/images/000/000/359/landscape/expressjslogo.png"},
    {name:"ReactJS", link:"data:image/png;base64,"},
    {name:"NodeJS", link:"https://image.ibb.co/gAAFiQ/nodejs.png"},
    {name:"jQuery", link:"https://image.ibb.co/m9YCV5/jquery.png"}
]

const languagesData = [
    {name: "HTML", link:"http://icons.iconarchive.com/icons/cornmanthe3rd/plex/256/Other-html-5-icon.png"},
    {name:"CSS", link:"https://image.ibb.co/mDL4ck/css3.png"},
    {name:"JavaScript", link:"https://preview.ibb.co/fMieA5/javascript.png"},
    {name:"C", link:"https://thumb.ibb.co/nr9aiQ/C_language_sh_600x600.png"},
    {name:"C++", link:"https://image.ibb.co/dhNmOQ/cpp.png"},
    {name:"C#", link:"http://innovativeteams.net/wp-content/uploads/2017/12/csharp.png"}
]

const tooltipRender = (data) => {
    return <div>
    {data.map(function(item, i){
        return <Tooltip title={item.name} key={i}>
                    <IconButton aria-label={item.name} style={iconMargin}>
                        <Avatar alt={item.name} src={item.link}/>
                    </IconButton>
                </Tooltip>
    })}
    </div>
}

export const languages = (tooltipRender(languagesData))
export const frameworkLibrary = (tooltipRender(frameworkLibraryData))
export const devTools = (tooltipRender(devToolsData))
export const other = (tooltipRender(otherData))


export const basicView = (
    <div>
    <ListItem>
                <ListItemIcon>
                    <LanguageIcon />
                </ListItemIcon>
                <ListItemText primary="Web Development" />
                </ListItem>
                
                
                <ListItem>
                <ListItemIcon>
                    <PhotoSizeIcon />
                </ListItemIcon>
                <ListItemText primary="Graphic Design" />
                </ListItem>
                
                <ListItem>
                <ListItemIcon>
                    <EditIcon />
                </ListItemIcon>
                <ListItemText primary="Content Writing" />
                </ListItem>

    </div>
)
  